 ...
 *
 * <p>The api here is a bit redundant, with similarly named methods returning either <code>void
 * </code> or {@link StyleElement} &mdash; e.g., {@link #inject(String) void inject(String)} v.
 * {@link #injectStylesheet(String) StyleElement injectStylesheet(String)}. The methods that return
 * {@link StyleElement} are not guaranteed to work as expected on Internet Explorer. Because they
 * are still useful to developers on other browsers they are not deprecated, but <strong>IE
 * developers should avoid the methods with {@link StyleElement} return values</strong> (at least up
 * until, and excluding, IE10).
 */
public class StyleInjector {

  /**
   * The DOM-compatible way of adding stylesheets. This implementation requires the host HTML page
   * to have a head element defined.
   *
   * <p>Deprecated, as this is the only implementation, this may be removed in a future release.
   */
  @Deprecated
  public static class StyleInjectorImpl {
    private static final StyleInjectorImpl IMPL = new StyleInjectorImpl();
  ...
  private static final List<String> toInject = new ArrayList<>();
  private static final List<String> toInjectAtEnd = new ArrayList<>();
  private static final List<String> toInjectAtStart = new ArrayList<>();
  ...
  public static void injectAtEnd(String css, boolean immediate) {
    toInjectAtEnd.add(css == null ? "" : css);
    inject(immediate);
  }
  ...
  public static void injectAtStart(String css, boolean immediate) {
    toInjectAtStart.add(0, css == null ? "" : css);
    inject(immediate);
  }
   ...
   * method. Call {@link #inject(String)} instead.</strong>
   *
  ...
  public static StyleElement injectStylesheet(String contents) {
    toInject.add(contents == null ? "" : contents);
   ...
   * <p>The returned StyleElement cannot be implemented consistently across all browsers.
   * Specifically, <strong>applications that need to run on Internet Explorer should not use this
   * method. Call {@link #injectAtEnd(String)} instead.</strong>
   *
   * @param contents the CSS contents of the stylesheet
   * @return the StyleElement that contains the newly-injected CSS (unreliable on Internet Explorer)
   */
  public static StyleElement injectStylesheetAtEnd(String contents) {
    toInjectAtEnd.add(contents == null ? "" : contents);
  ...
  public static StyleElement injectStylesheetAtStart(String contents) {
    toInjectAtStart.add(0, contents == null ? "" : contents);
   ...
   *
   * <p>This method should be used with some caution as StyleInjector may recycle StyleElements on
   * certain browsers. Specifically, <strong>applications that need to run on Internet Explorer
   * should not use this method. </strong>
   *
  ...
  private static StyleElement flush(List<String> which) {
    StyleElement toReturn = null;
    StyleElement maybeReturn;

    if (toInjectAtStart.size() != 0) {
      String css = String.join("", toInjectAtStart);
    ...
    if (toInject.size() != 0) {
      String css = String.join("", toInject);
    ...
    if (toInjectAtEnd.size() != 0) {
      String css = String.join("", toInjectAtEnd);
