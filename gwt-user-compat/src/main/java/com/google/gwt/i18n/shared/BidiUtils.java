...
public class BidiUtils {
   ...
   *
   * <p>This is volatile to prevent the compiler from inlining this constant in various references
   * below.
   */
  ...
  /** Regular expression to check if the first strongly directional character in a string is LTR. */
  ...
  /**
   * Simplified regular expression for an HTML tag (opening or closing) or an HTML escape. We might
   * want to skip over such expressions when estimating the text directionality.
   */
  ...
  /**
   * Regular expressions to check if the last strongly-directional character in a piece of text is
   * LTR.
   */
  ...
  /**
   * Like {@link #endsWithLtr(String, boolean)}, but assumes {@code str} is not HTML / HTML-escaped.
   */
  ...
  /**
   * Like {@link #endsWithRtl(String, boolean)}, but assumes {@code str} is not HTML / HTML-escaped.
   */
  ...
  /**
   * Like {@link #estimateDirection(String, boolean)}, but assumes {@code str} is not HTML /
   * HTML-escaped.
   */
  public Direction estimateDirection(String str) {
    ...
    return total == 0
        ? (hasWeaklyLtr ? Direction.LTR : Direction.DEFAULT)
        : ((float) rtlCount / total > RTL_DETECTION_THRESHOLD ? Direction.RTL : Direction.LTR);
  }

  /**
   * Estimates the directionality of a string based on relative word counts. If the number of RTL
   * words is above a certain percentage of the total number of strongly directional words, returns
   * RTL. Otherwise, if any words are strongly or weakly LTR, returns LTR. Otherwise, returns
   * DEFAULT, which is used to mean "neutral". Numbers are counted as weakly LTR.
   *
   * @param str the string to check
   * @param isHtml whether {@code str} is HTML / HTML-escaped. Use this to ignore HTML tags and
   *     escapes that would otherwise be mistaken for LTR text.
  ...
  /**
   * Like {@link #startsWithLtr(String, boolean)}, but assumes {@code str} is not HTML /
   * HTML-escaped.
   */
  ...
  /**
   * Like {@link #startsWithRtl(String, boolean)}, but assumes {@code str} is not HTML /
   * HTML-escaped.
   */
  ...
  /**
   * Returns the input text with spaces instead of HTML tags or HTML escapes, if isStripNeeded is
   * true. Else returns the input as is. Useful for text directionality estimation. Note: the
   * function should not be used in other contexts; it is not 100% correct, but rather a good-enough
   * implementation for directionality estimation purposes.
   */
