...
/**
 * Base class for {@link BidiFormatter} and {@link SafeHtmlBidiFormatter} that contains their common
 * implementation.
 */
public abstract class BidiFormatterBase {

  /**
   * Abstract factory class for BidiFormatterBase. BidiFormatterBase subclasses will usually have a
   * non-abstract inner Factory class subclassed from this one, and use a static member of that
   * class in order to prevent the needless creation of objects. For example, see {@link
   * BidiFormatter}.
   */
  protected abstract static class Factory<T extends BidiFormatterBase> {
    ...
    public abstract T createInstance(Direction contextDir, boolean alwaysSpan);

    public T getInstance(Direction contextDir, boolean alwaysSpan) {
    ...
    private int calculateIndex(Direction contextDir, boolean alwaysSpan) {
      int i = contextDir == Direction.LTR ? 0 : contextDir == Direction.RTL ? 1 : 2;
      if (alwaysSpan) {
        i += 3;
      }
      return i;
  ...
  /**
   * Estimates the direction of a string using the best known general-purpose method, i.e. using
   * relative word counts. Direction.DEFAULT return value indicates completely neutral input.
   *
   * @param str String whose direction is to be estimated
  ...
  /**
   * Returns whether the span structure added by the formatter should be stable, i.e., spans added
   * even when the direction does not need to be declared.
   */
  ...
  protected String knownDirAttrBase(Direction dir) {
    if (dir != contextDir) {
      return dir == Direction.LTR ? "dir=ltr" : dir == Direction.RTL ? "dir=rtl" : "";
    }
    return "";
  }
  ...
  protected String markAfterBase(String str, boolean isHtml) {
    ...
    return dirResetIfNeeded(str, BidiUtils.get().estimateDirection(str), false, true);
  }
  ...
  protected String markBase() {
    return contextDir == Direction.LTR
        ? Format.LRM_STRING
   ...
   * @return Input string after applying the above processing.
   */
  protected String unicodeWrapWithKnownDirBase(
  ...
  /**
   * Returns a unicode BiDi mark matching the context direction (LRM or RLM) if {@code dirReset},
   * and if the overall direction or the exit direction of {@code str} are opposite to the context
   * direction. Otherwise returns the empty string.
   *
   * @param str The input string
   ...
   */
  private String dirResetIfNeeded(String str, Direction dir, boolean isHtml, boolean dirReset) {
    // endsWithRtl and endsWithLtr are called only if needed (short-circuit).
    if (dirReset
        && ((contextDir == Direction.LTR
                && (dir == Direction.RTL || BidiUtils.get().endsWithRtl(str, isHtml)))
            || (contextDir == Direction.RTL
                && (dir == Direction.LTR || BidiUtils.get().endsWithLtr(str, isHtml))))) {
