...
public class SimpleEventBus extends EventBus {
   ...
   *     not honored for handlers tied to a specific event source (via {@link #addHandlerToSource}.
   */
  @Deprecated
  ...
  protected <H> H getHandler(Event.Type<H> type, int index) {
    assert index < getHandlerCount(type)
        : "handlers for "
            + type.getClass()
            + " have size: "
            + getHandlerCount(type)
            + " so do not have a handler at index: "
            + index;
